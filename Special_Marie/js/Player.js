		...
		switch (direction) {
			...
			case "up":
				for (let i = 0; i < this.game.map.dataCenter.length; i++) {
					if (this.dataCenter.map.x - this.game.map.dataCenter[i].map_x > 0 &&
						this.dataCenter.map.x - this.game.map.dataCenter[i].map_x < 30 &&
						this.dataCenter.map.y - this.game.map.dataCenter[i].map_y < 35 && this.dataCenter.map.y - this.game.map.dataCenter[i].map_y > 0) {
						console.log(i, "我碰到砖头了", this.dataCenter.map.y, this.game.map.dataCenter[i].map_y);
						console.log(i,this.dataCenter.map.x);
						this.game.map.dataCenter[i].map_y -= this.game.map.dataCenter[i].mapSize_y;
						// return false;
					} else {
						// return true;
					}
